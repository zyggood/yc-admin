...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    private Set<String> getUserPermissions(Long userId) {
        ...
        try {
            // 使用权限API服务计算用户权限（内部会处理超级管理员的权限继承逻辑）
            Set<String> userPermissions = permissionApiService.calculateUserPermissions(userId);
            permissions.addAll(userPermissions);
            log.debug("用户 {} 权限: {}", userId, userPermissions);
        } catch (Exception e) {
